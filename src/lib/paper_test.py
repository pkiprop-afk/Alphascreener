...
def run_backtest_from_signals(price_df: pd.DataFrame, signal_df: pd.DataFrame, risk_reward: float = 2.0, max_holding_bars: int = 12) -> tuple[pd.DataFrame, dict] :
    ...
    for idx in range(len(signal_df) - 1):
        ...
        swing_level = row.get("previous_swing_low") if is_long else row.get("previous_swing_highs")
        ...
        resolved = resolve_trade(signal_df, trade, max_holding_bars)
        trades.append(resolved)
    
    trades_df = pd.DataFrame(trades)
    return trades_df, summarize_backtest(trades)
